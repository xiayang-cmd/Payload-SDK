...
T_DjiReturnCode SimpleFcSubscription::startService()
{
    ...
    rc = DjiFcSubscription_SubscribeTopic(
             DJI_FC_SUBSCRIPTION_TOPIC_POSITION_FUSED, DJI_DATA_SUBSCRIPTION_TOPIC_10_HZ, nullptr);
    if (rc) return rc;

    rc = DjiFcSubscription_SubscribeTopic(
             DJI_FC_SUBSCRIPTION_TOPIC_ALTITUDE_FUSED,  DJI_DATA_SUBSCRIPTION_TOPIC_10_HZ, nullptr);
    if (rc) return rc;

    rc = DjiFcSubscription_SubscribeTopic(
             DJI_FC_SUBSCRIPTION_TOPIC_HEIGHT_FUSION,   DJI_DATA_SUBSCRIPTION_TOPIC_10_HZ, nullptr);
    if (rc) return rc;

    rc = DjiFcSubscription_SubscribeTopic(
             DJI_FC_SUBSCRIPTION_TOPIC_QUATERNION,      DJI_DATA_SUBSCRIPTION_TOPIC_10_HZ, nullptr);
    if (rc) return rc;

    rc = DjiFcSubscription_SubscribeTopic(
             DJI_FC_SUBSCRIPTION_TOPIC_VELOCITY,        DJI_DATA_SUBSCRIPTION_TOPIC_10_HZ, nullptr);
    return rc;
}
