...
void SimpleMsgHandler::handleDownlinkMessage(const uint8_t* data, std::size_t length)
{
    ...
    if (chk != CheckResult::Ok) {
        printf("[SimpleMsgHandler] checkDownlink failed, code=%d\n", static_cast<int>(chk));
        return;
    }
    ...
    case Command::BoardPos:
        ...
        break;
    default:
        printf("[SimpleMsgHandler] Unhandled cmd=0x%02X\n", hdr->cmd);
        break;
    }
...
{
    if (payload.drone_id != g_drone_id) {
        return;
    }
    g_role = DroneRole::Master; // 设置为主机角色
    printf("[SetHostMode] drone_id=%u\n", payload.drone_id);
    /* TODO: your code here */
}
...
void SimpleMsgHandler::handleSetSlaveMode(const Downlink::PayloadDroneId& payload)
{
    ...
    printf("[SetSlaveMode] drone_id=%u\n", payload.drone_id);
}
...
void SimpleMsgHandler::handleSetOffset(const Downlink::PayloadOffset& payload)
{
    ...
    printf("[SetOffset] drone_id=%u, offset=(%u,%u,%u)\n",
           payload.drone_id,
           payload.offset_x, payload.offset_y, payload.offset_z);
    g_offset_x = static_cast<float>(payload.offset_x) / 100.0f; // 假设单位为 cm
    g_offset_y = static_cast<float>(payload.offset_y) / 100.0f; // 假设单位为 cm
    g_offset_z = static_cast<float>(payload.offset_z) / 100.0f; // 假设单位为 cm

    if(isOffsetReliable()){
        printf("offset is not reliable.\n");
    }
}   

void SimpleMsgHandler::handleBoardPos(const Downlink::PayloadPosition& payload)
{
    printf("uav_id=%u, Lat=%.7f, Lon=%.7f, Alt=%.1f m\n",
           payload.drone_id,
           payload.latitude_deg, payload.longitude_deg, payload.altitude_fused);
    ...
    if (g_host_confirmed) {
        if (payload.drone_id == g_host_drone_id) {
            ...
            candidate_id     = payload.drone_id;
            candidate_cnt    = 1;
            printf("[HOST LOST] switch to candidate id=%u, counting...\n", candidate_id);
        }
    }
}
